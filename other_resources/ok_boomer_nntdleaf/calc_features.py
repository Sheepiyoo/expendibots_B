...
def get_nn_features(state):
    grid_format = game.get_grid_format(state)
    features = []

    for x in range (0, 8):
        for y in range(0, 8):
            if (x,y) not in grid_format.keys(): features.append(0)
            else:
                col = grid_format[(x,y)][0]
                num = int(grid_format[(x,y)][1:])
                if col == "w'": features.append(num/12)
                else: features.append(-num/12)
    return features
